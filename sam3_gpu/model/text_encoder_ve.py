...
class ResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        # Attention
        self.attn = nn.MultiheadAttention(d_model, n_head, bias=True, batch_first=True)
    ...
    def attention(
        ...
        attn_output, _ = self.attn(q_x, k_x, v_x, attn_mask=attn_mask)
        return attn_output
...
class TextTransformer(nn.Module):
    def __init__(
        ...
        self.positional_embedding = nn.Parameter(
            torch.zeros(self.num_pos, width)
        )
        ...
        if no_causal_mask:
            ...
            self.register_buffer("attn_mask", self.build_causal_mask())

        if proj_bias:
            self.text_projection = nn.Linear(width, output_dim)
        else:
            self.text_projection = nn.Parameter(
                torch.zeros(width, output_dim)
            )

    def build_causal_mask(self) -> torch.Tensor:
        # lazily create causal attention mask, with full attention between the tokens
        # pytorch uses additive attention mask; fill with -inf
        mask = torch.full((self.num_pos, self.num_pos), -float("inf"))
...
class VETextEncoder(nn.Module):
    ...
    def forward(
        ...
        if isinstance(text[0], str):
            ...
            text_attention_mask = (tokenized != 0).to(torch.bool)
            ...
            text_attention_mask = torch.ne(text_attention_mask, 1)
