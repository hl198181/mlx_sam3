...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        memory_text: Optional[torch.Tensor] = None,  # num_token, bs, d_model
        ...
        **kwargs,  # additional kwargs for compatibility
        ...
        if self.self_attn is not None:
            ...
            q = k = self.with_pos_embed(tgt_o2o, tgt_query_pos_o2o).permute(1, 0, 2)
            tgt2 = self.self_attn(q, k, tgt_o2o.permute(1, 0, 2), attn_mask=self_attn_mask)[0].permute(1, 0, 2)
        ...
        if self.use_text_cross_attention:
            ...
            tgt2 = self.ca_text(
                self.with_pos_embed(tgt, tgt_query_pos).permute(1, 0, 2),
                memory_text,
                memory_text,
        ...
        tgt2 = self.cross_attn(
            self.with_pos_embed(tgt, tgt_query_pos).permute(1, 0, 2),
            self.with_pos_embed(memory, memory_pos).permute(1, 0, 2),
            memory.permute(1, 0, 2),
            attn_mask=cross_attn_mask,
            key_padding_mask=(
                memory_key_padding_mask.permute(1, 0)
...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        if boxRPB != "none":
            ...
            if resolution is not None and stride is not None:
                ...
                self.register_buffer('_cached_coords_h', coords_h, persistent=False)
                self.register_buffer('_cached_coords_w', coords_w, persistent=False)
                self.compilable_cord_cache = (feat_size, feat_size)
    ...
    def _get_coords(H, W):
        ...
        coords_w = torch.arange(0, W, dtype=torch.float32) / W
    ...
    def _get_rpb_matrix(self, reference_boxes, feat_size):
        ...
        if self.compilable_cord_cache == feat_size and hasattr(self, '_cached_coords_h'):
            coords_h = self._cached_coords_h
            coords_w = self._cached_coords_w
        else:
    ...
    def forward(
        ...
        reference_boxes: Optional[torch.Tensor] = None,  # num_queries, bs, 4
        # for memory
        ...
        if self.presence_token is not None and is_instance_prompt is False:
            presence_out = self.presence_token.weight[None].expand(1, bs, self.d_model)
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            if self.box_refine:
                ...
                new_reference_points = torch.sigmoid(outputs_unsig)
            ...
            if self.presence_token is not None and is_instance_prompt is False:
                ...
                if self.clamp_presence_logits:
                    intermediate_layer_presence_logits = torch.clamp(
