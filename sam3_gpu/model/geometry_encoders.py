...
def is_right_padded(mask):
    ...
    if mask.numel() == 0 or (len(mask.shape) > 0 and mask.shape[-1] == 0):
        return True

    mask_int = mask.to(torch.int64)
    sorted_mask, _ = torch.sort(mask_int, dim=-1)
    return torch.all(mask_int == sorted_mask)
...
def concat_padded_sequences(seq1, mask1, seq2, mask2, return_index: bool = False):
    ...
    concatenated_mask = (
        torch.arange(max_length, device=seq1.device)[None].repeat(batch_size, 1)
        >= final_lengths[:, None]
    )
    # (max_len, batch_size, hidden_size)
    concatenated_sequence = torch.zeros(
        (max_length, batch_size, hidden_size), dtype=seq2.dtype, device=seq2.device
    )
...
class SequenceGeometryEncoder(nn.Module):
    ...
    def _encode_points(self, points, points_mask, points_labels, img_feats):
        ...
        if self.points_pool_project is not None:
            ...
            grid = points.permute(1, 0, 2)[:, :, None]
            grid = (grid * 2) - 1
            sampled = F.grid_sample(
                img_feats, grid, mode='bilinear', padding_mode='zeros', align_corners=False
            )
        ...
        type_embed = self.label_embed(points_labels.to(torch.int64))
    ...
    def _encode_boxes(self, boxes, boxes_mask, boxes_labels, img_feats):
        ...
        if self.boxes_pool_project is not None:
            ...
            # Build list of box tensors per batch element for roi_align
            boxes_inp = [boxes_xyxy[:, i, :].to(torch.float32) for i in range(bs)]
            sampled = torchvision.ops.roi_align(
                img_feats, boxes_inp, output_size=(self.roi_size, self.roi_size)
            )
            ...
            proj = proj.reshape(bs, n_boxes, self.d_model).permute(1, 0, 2)
        ...
        type_embed = self.label_embed(boxes_labels.to(torch.int64))
    ...
    def _encode_masks(
        ...
        if self.add_mask_label:
            masks = masks + self.mask_label_embed(mask_labels.to(torch.int64))
    ...
    def forward(self, geo_prompt: Prompt, img_feats, img_sizes, img_pos_embeds=None):
        ...
        if self.encode_boxes_as_points:
            ...
            boxes_xyxy = box_cxcywh_to_xyxy(boxes)
            top_left, bottom_right = torch.chunk(boxes_xyxy, 2, dim=-1)
        ...
        if self.cls_embed is not None:
            cls = self.cls_embed.weight.reshape(1, 1, self.d_model).repeat(1, bs, 1)
        ...
        if self.encode is not None:
            for lay in self.encode:
                final_embeds = lay(
                    tgt=final_embeds.permute(1, 0, 2),
                    memory=seq_first_img_feats.permute(1, 0, 2),
                    tgt_key_padding_mask=final_mask,
                    pos=seq_first_img_pos_embeds.permute(1, 0, 2),
                ).permute(1, 0, 2)
