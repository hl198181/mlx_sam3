...
class SAM3VLBackbone(nn.Module):
    def __init__(self, visual, text, compile_visual=False, act_ckpt_whole_vision_backbone=False, act_ckpt_whole_language_backbone=False, scalp=0):
        super().__init__()
    ...
    def call_image(self, samples):
        return self._call_image_no_ack_ckpt(samples=samples)
    ...
    def _call_image_no_ack_ckpt(self, samples):
        ...
        if self.scalp > 0:
            ...
            if sam2_features is not None and sam2_pos is not None:
                sam2_features, sam2_pos = sam2_features[:-self.scalp], sam2_pos[:-self.scalp]
        sam2_output = None
        if sam2_features is not None and sam2_pos is not None:
        ...
        output = {"vision_features": sam3_src, "vision_pos_enc": sam3_pos, "backbone_fpn": sam3_features, "sam2_backbone_out": sam2_output}
        return output
    ...
    def _call_text_no_ack_ckpt(self, captions, input_boxes=None, additional_text=None):
        output = {}
