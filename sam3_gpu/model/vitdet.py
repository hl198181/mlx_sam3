...
def view_as_complex(x: torch.Tensor) -> torch.Tensor:
    ...
    parts = x.reshape(new_shape)
    return torch.view_as_complex(parts.contiguous())
...
def init_t_xy(
    ...
    t_x = (t % end_x).to(torch.float32)
    t_y = torch.div(t, end_x, rounding_mode='floor').to(torch.float32)
...
def compute_axial_cis(
    ...
    freqs_x = 1.0 / (theta ** (torch.arange(0, dim, 4)[: (dim // 4)].to(torch.float32) / dim))
    freqs_y = 1.0 / (theta ** (torch.arange(0, dim, 4)[: (dim // 4)].to(torch.float32) / dim))

    t_x, t_y = init_t_xy(end_x, end_y, scale=scale_pos, offset=offset)
    freqs_x = torch.outer(t_x, freqs_x)
    freqs_y = torch.outer(t_y, freqs_y)
    freqs_cis_x = polar(torch.ones_like(freqs_x), freqs_x)
    freqs_cis_y = polar(torch.ones_like(freqs_y), freqs_y)
...
def apply_rotary_enc(
    ...
    xq_ = view_as_complex(xq.float())
    xk_ = (view_as_complex(xk.float())
           if xk.shape[-2] != 0
           else None
    )
    ...
    if xk_ is None:
        return xq_out.to(xq.dtype), xk

    if repeat_freqs_k:
        r = xk_.shape[-2] // xq_.shape[-2]
        reps = [1] * (freqs_cis.ndim - 2) + [r, 1]
        freqs_cis = freqs_cis.repeat(*reps)
...
def get_abs_pos(
    ...
    if size != h or size != w:
        ...
        if tiling:
            ...
            new_abs_pos = new_abs_pos.repeat(1, 1, rep_h, rep_w)
            new_abs_pos = new_abs_pos[:, :, :h, :w]
        else:
            scale_h = h / new_abs_pos.shape[2]
            scale_w = w / new_abs_pos.shape[3]
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: B C H W (NCHW) - PyTorch native
        x = self.proj(x)
        # B C H W -> B H W C for ViT blocks
        x = x.permute(0, 2, 3, 1)
        return x
...
class Attention(nn.Module):
    ...
    def _setup_rel_pos(self, rel_pos_zero_init: bool = True) -> None:
        ...
        if not rel_pos_zero_init:
            nn.init.trunc_normal_(self.rel_pos_h, std=0.02, a=-2, b=2)
            nn.init.trunc_normal_(self.rel_pos_w, std=0.02, a=-2, b=2)
        ...
        self.register_buffer("relative_coords", relative_coords.to(torch.int64))
    ...
    def _setup_rope_freqs(self) -> None:
        ...
        if self.cls_token:
            ...
            cls_freqs_cis = polar(torch.ones_like(t), t)[None, :]
            freqs_cis = torch.cat([cls_freqs_cis, freqs_cis], dim=0)

        self.register_buffer("freqs_cis", freqs_cis)

    def _apply_rope(self, q, k) -> Tuple[torch.Tensor, torch.Tensor]:
        if not self.use_rope:
            return q, k
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if ndim == 4:
            ...
            new_shape = (B, self.num_heads, H_int, W_int, -1)
            x = (
                x.reshape(new_shape)
                .permute(0, 2, 3, 1, 4)
                .reshape(B, H_int, W_int, -1)
            )
        else:
...
class ViT(nn.Module):
    def __init__(
        ...
        if self.pos_embed is not None:
            nn.init.trunc_normal_(self.pos_embed, std=0.02, a=-2, b=2)
    ...
    def forward(self, x: torch.Tensor) -> List[torch.Tensor]:
        ...
        s = 0
        if self.retain_cls_token:
            cls_embed = self.class_embedding.expand(x.shape[0], -1, -1)
            x = torch.cat([cls_embed, x.flatten(1, 2)], dim=1)
            s = 1
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if (i == self.full_attn_ids[-1]) or (
                ...
                if feats.ndim == 4:
                    ...
                    assert feats.ndim == 3
                    h_int = w_int = int(math.sqrt(feats.shape[1]))
                    feats = feats.reshape(
                        feats.shape[0], h_int, w_int, feats.shape[-1]
                    ).permute(0, 3, 1, 2)
